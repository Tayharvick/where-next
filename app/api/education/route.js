import { resolveTownEducation } from "@/lib/townEducation";
...
export async function POST(req) {
  try {
    ...
    const data = await resolveTownEducation(town, stateAbbr);
    return Response.json(data, {
      headers: { "Cache-Control": "public, s-maxage=86400, stale-while-revalidate=604800" },
    });
  } catch (err) {
    return Response.json(
      { ok: false, error: err?.message || "Education lookup failed" },
      { status: 502 }
    );
  }
}
...
export async function GET(req) {
  ...
  try {
    const data = await resolveTownEducation(town, state || parseTownGeo(town, state).state);
    return Response.json(data, {
      headers: { "Cache-Control": "public, s-maxage=86400, stale-while-revalidate=604800" },
    });
  } catch (err) {
